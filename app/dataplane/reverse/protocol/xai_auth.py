...
async def _grpc_call(
    ...
    *,
    ...
    if status.ok or status.code == -1:
        ...
        logger.debug(
            "auth grpc call completed: label={} grpc_code={}", label, status.code
        )
    else:
        if not shared:
            await proxy.feedback(
                lease,
                ProxyFeedback(
                    kind=ProxyFeedbackKind.UPSTREAM_5XX, status_code=status.http_equiv
                ),
            )
        raise UpstreamError(
        ...
        async with ResettableSession(**kwargs) as session:
            try:
                ...
                if exc.status == 429 and "birth-date-change-limit-reached" in body:
                    logger.debug(
                        "auth birth date already set (locked), skipping: token={}...",
                        token[:8],
                    )
                else:
                    raise
