    ...
    async def _solve(
        ...
        *,
        ...
        try:
            ...
            final_url = solution.get("url", target)
            host = (
                urlparse(final_url).hostname or urlparse(target).hostname or ""
            ).lower()
            ...
            if host != target_host:
                logger.warning(
                    "flaresolverr redirect detected: target={} final_url={} "
            ...
            filtered = [
                ...
                if not host
                or not cookie.get("domain")
                or host.endswith(str(cookie.get("domain", "")).lstrip(".").lower())
            ]
