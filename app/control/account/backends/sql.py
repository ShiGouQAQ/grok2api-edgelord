    ...
    async def reset_expired_console_windows(self) -> int:
        ...
        if self._dialect == "postgresql":
            count_sql = sa.text(
                ...
                " AND (quota_console->>'source' IS NULL OR (quota_console->>'source')::int != 1)"
                " AND ("
                "   ("
            ...
            count_sql = sa.text(
                ...
                " WHERE status = 'active'"
                " AND deleted_at IS NULL"
                " AND (JSON_EXTRACT(quota_console, '$.source') IS NULL"
                "      OR CAST(JSON_EXTRACT(quota_console, '$.source') AS SIGNED) != 1)"
                " AND ("
                "   ("
