...
class MessagesRequest(BaseModel):
    ...
    thinking: Any = (
        None  # {type:"enabled", budget_tokens:N} — used to enable thinking output
    )
...
async def _safe_sse_anthropic(stream):
    ...
    try:
        ...
        # Pass through upstream error type/code instead of hardcoding
        # "api_error" — ported from Go commit 982e27b.
        # Single except block (not AppError + Exception) because
        # UpstreamError(AppError) — separated except blocks would
        # make isinstance checks in the second block unreachable.
        if isinstance(exc, AppError):
            err = exc.to_dict()["error"]
            if isinstance(exc, UpstreamError) and exc.upstream_code:
                err["type"] = exc.upstream_code
