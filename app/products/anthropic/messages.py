...
def _extract_system_text(system: Any) -> str:
    """Extract text from Anthropic system value (string or text-block list)."""
    if not system:
        return ""
...
def _parse_anthropic_messages(
    ...
    # Merge top-level system with inline role=system from messages array
    # (Claude Code injects system messages mid-conversation).
    system_parts: list[str] = []
    top_level_text = _extract_system_text(system)
    if top_level_text.strip():
        system_parts.append(top_level_text)

    remaining: list[dict] = []
    for msg in messages:
        role = msg.get("role", "user")
        if role == "system":
            text = _extract_system_text(msg.get("content", ""))
            if text.strip():
    ...
    if system_parts:
        internal.append({"role": "system", "content": "\n\n".join(system_parts)})
