...
**DPoP 二次修复（2026-08-05，Authorization header + 审计跟进）**：
1. **Authorization: Bearer anonymous 删除**（根因 #2）：`build_console_headers` 原在无 DPoP 参数时写死 `Authorization: Bearer anonymous`（Python 独创，Go `applyBrowserHeaders` 从不发送）→ token 交换请求带多余 header 被 CF 200 挑战页拦截 → body 非 JSON → DPoPError → 502。已删该 else 分支（headers.py:382-385）+ docstring 修正；回归测试 `test_chat_dpop_exchange_has_no_authorization`。Go 参考：console_test.go serveTestDPoPToken 显式断言 token 请求无 Authorization/DPoP。
2. **审计发现修复（4 个并行审计 Agent）**：
   - CRITICAL: `fetch_console_usage` 的 GET /v1/usage 缺浏览器 headers（Cookie/UA/Origin）——`DPoPSessionManager` 未传 `browser_headers`，Go doDPoPRequest 对每个请求 applyBrowserHeaders → 已加 `browser_headers=lambda lease: build_console_headers(token, lease=lease)`。
   - CRITICAL: DPoP token 403 非 definitive-block 未真正失效 clearance bundle（Go `lease.InvalidateClearance()`）→ `ProxyFeedback` 新增 `invalidate_clearance` 标志，`feedback()` 失效集合扩展该标志，chat token 403 分支 + usage `_handle_usage_status`/`_handle_dpop_token_error` 403 分支置位。
   - MEDIUM: SSE `response.failed` 事件被忽略（Go 将 error/response.failed 同为流错误）→ ConsoleStreamAdapter 已捕获并抛 UpstreamError。
