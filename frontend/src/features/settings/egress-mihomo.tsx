...
import { ArrowRightLeft, Eraser, RefreshCw } from "lucide-react";
...
import { clearMihomoBlacklist, getMihomoStatus, refreshMihomoDelays, switchMihomoNode, type MihomoMemberDTO } from "@/features/settings/settings-api";
...
export function MihomoStatusPanel() {
  ...
  const refreshDelays = useMutation({
    mutationFn: refreshMihomoDelays,
    onSuccess: () => { invalidate(); toast.success(t("settings.egress.mihomoDelaysRefreshed")); },
    onError: (error) => { invalidate(); toast.error(error.message); },
  });
          ...
          {!query.data.enabled ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Tooltip><TooltipTrigger asChild><Button type="button" size="sm" variant="secondary" disabled={refreshDelays.isPending} onClick={() => refreshDelays.mutate()}>{refreshDelays.isPending ? <Spinner /> : <RefreshCw />}{t("settings.egress.mihomoRefreshDelays")}</Button></TooltipTrigger><TooltipContent>{t("settings.egress.mihomoRefreshDelaysHelp")}</TooltipContent></Tooltip>
              </div>
