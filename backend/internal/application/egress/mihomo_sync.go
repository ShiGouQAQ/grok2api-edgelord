...
// IsManagedSourceName 报告订阅源是否为系统托管（Mihomo 测试组成员同步源）。
// 托管源不可经管理端创建/编辑/删除/同步——其生命周期由 MihomoSyncer 全权
// 管理，误删会置空成员节点的 source 归属，误改名会导致同步重建孤儿源。
func IsManagedSourceName(name string) bool {
	return strings.TrimSpace(name) == mihomoSyncSourceName
}
