...
// TestMihomoDelayProbeURLRoundTrip 验证 DelayProbeURL 配置全链路往返：默认
// Apple captive 端点（config.DefaultMihomoDelayProbeURL）→ applyDomainConfig
// 落库 → toEditable 暴露 → mergeEditable 保留。
func TestMihomoDelayProbeURLRoundTrip(t *testing.T) {
	base := testConfig(t)
	if base.Provider.Web.MihomoDelayProbeURL != config.DefaultMihomoDelayProbeURL {
		t.Fatalf("default mihomoDelayProbeURL = %q, want %q", base.Provider.Web.MihomoDelayProbeURL, config.DefaultMihomoDelayProbeURL)
	}
