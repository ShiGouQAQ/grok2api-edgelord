...
// TestProbeGroupDelaysSlowGroup 验证整组延迟探测使用独立的长超时 client：
// mihomo 对几十个节点的组测延迟总耗时（6s）超过通用 client 的 5s 总超时，
// 组级探测必须仍能成功返回，而不是被提前掐断。
func TestProbeGroupDelaysSlowGroup(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		time.Sleep(6 * time.Second)
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]int{"slow": 300, "fast": 50})
	}))
	defer server.Close()

	client := NewMihomoClient(MihomoConfig{Enabled: true, APIURL: server.URL, GroupName: "XAI-GROUP"})
	delays, err := client.ProbeGroupDelays(context.Background(), "XAI-GROUP", "http://www.gstatic.com/generate_204", 5*time.Second)
	if err != nil {
		t.Fatalf("ProbeGroupDelays with slow group: %v", err)
	}
	if delays["fast"] != 50 || delays["slow"] != 300 {
		t.Fatalf("unexpected delays: %v", delays)
	}
}

// TestProbeGroupDelaysStripsCallerDeadline 锁住根因：调用方（如状态 API）
// 常带 3s 短 deadline，而整组探测需 5-8s。探测必须剥离调用方 deadline
// （context.WithoutCancel），否则必然被掐断。
func TestProbeGroupDelaysStripsCallerDeadline(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		time.Sleep(6 * time.Second)
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]int{"slow": 300, "fast": 50})
	...
	defer cancel()
	delays, err := client.ProbeGroupDelays(ctx, "XAI-GROUP", "http://www.gstatic.com/generate_204", 5*time.Second)
	if err != nil {
		t.Fatalf("ProbeGroupDelays with short caller deadline: %v", err)
	}
	if delays["fast"] != 50 || delays["slow"] != 300 {
		t.Fatalf("unexpected delays: %v", delays)
	}
