"""Tests for UpstreamError mapper methods (to_feedback_kind, to_proxy_feedback_kind)."""
...
# ---------------------------------------------------------------------------
# Scenario data: (row#, kwargs, expected_fb, expected_pfb)
# ---------------------------------------------------------------------------

SCENARIOS: list[tuple[int, dict, FeedbackKind, ProxyFeedbackKind]] = [
    # 1: 401 bare
    (
        1,
        {"status": 401},
...
class Test403PermanentDenialDivergence:
    """403+permanent_account_denial: to_feedback_kind=FORBIDDEN, to_proxy_feedback_kind=FORBIDDEN.

    Intentional: state machine sees 'access denied' (don't retry),
    proxy layer sees 'forbidden' (don't switch nodes).
    """
    ...
    def test_divergence_without_flag_is_consistent(self) -> None:
        """Plain 403 (no permanent_account_denial) → both agree on FORBIDDEN."""
        err = _err(status=403)
